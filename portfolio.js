...
function handleSubmit(e){
  ...
  fetch('https://formspree.io/f/xaqknanp', {
    method: 'POST',
  ...
  .then(response => {
    if(response.ok){
      ...
      s.innerHTML = '✅ Thanks for reaching out, <strong>'+name+'</strong>! I\'ve received your message and will get back to you at <strong>'+email+'</strong> within 24 hours. Looking forward to chatting!';
      s.classList.add('show');
    } else {
      // Formspree returned an error
      btn.disabled = false;
      btn.querySelector('span').textContent = 'Send Message ✉';
      alert('Something went wrong. Please try again or email me directly at ibraheemkhalee.ckp@gmail.com');
    }
  })
  .catch(() => {
    // Network error
